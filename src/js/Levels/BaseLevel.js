...
class BaseLevel extends React.Component {
    ...
    initWorld() {
        this.game.engine = Matter.Engine.create({
            ...
            enableSleeping: true,
            world: Matter.World.create(this.game.world),
        });
        ...
        var wallRight = Matter.Bodies.rectangle(
            this.game.settings.width + 40,
            this.game.settings.height / 2,
            90,
        ...
        this.game.goals.map((goal, key) => {
            ...
            if (goal.type == 'rectangle') {
                this.game.goals[key].object = Matter.Bodies.rectangle(goal.position.x, goal.position.y, goal.width, goal.height, options);
            } else {
                this.game.goals[key].object = Matter.Bodies.circle(goal.position.x, goal.position.y, 40, options);
            }
    ...
    addPlayer() {
        ...
        Matter.World.add(this.game.engine.world, [this.game.rock, this.game.elastic]);
        ...
        Matter.Events.on(this.game.engine, 'afterUpdate', () => {
            if (this.game.mouseConstraint.mouse.button === -1 && this.game.rock.position.y < this.game.settings.player.position.y - 20) {
